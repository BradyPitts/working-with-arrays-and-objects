...
mixedNumbers.filter(evenNumbers)
...
//Code Here
let postTaxPrices = (arr) =>{
  return (arr *= 1.07);
}

prices.map(postTaxPrices);
...
//Code Here
let totalPopulation = (total, arr) =>{
  return (total + arr);
}

populations.reduce(totalPopulation)
...
monstersInYourPocket.filter(myStrongest)
...
let bobsTotal = (total, arr) =>{
  if(arr.owner == 'Bob'){
    return (total + arr.price);
  }
}

purchases.reduce(bobsTotal)